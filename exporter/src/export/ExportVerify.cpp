...
static void CheckBitMapAndVideoNum(const std::shared_ptr<PAGExportSession>& session,
                                   std::vector<pag::Composition*>& compositions,
                                   std::vector<pag::ImageBytes*>& images) {
  constexpr int maxImageNum = 30;
  constexpr int maxBmpCompositionNum = 3;

  int staticVideoSequenceNum = 0;
  int sequenceCompositionNum = 0;
  ...
  int imageNum = static_cast<int>(images.size()) + staticVideoSequenceNum;
...
static void GetLayerTimeRangeAndEffectCount(const std::shared_ptr<PAGExportSession>&,
...
static void CheckEffect(const std::shared_ptr<PAGExportSession>& session,
  ...
  Helper::TraversalLayers(session, composition, pag::LayerType::Unknown, 0,
...
static void GetLayerVideoTrackTimeRangeAndCount(const std::shared_ptr<PAGExportSession>&,
                                                pag::Layer* layer, pag::Frame compositionStartTime,
                                                void* ctx) {
...
static void GetNameOfLayerWithVideoTrack(const std::shared_ptr<PAGExportSession>&,
                                         pag::Layer* layer, pag::Frame compositionStartTime,
                                         void* ctx) {
  if (!isLayerHasVideoTrack(layer)) {
    return;
...
static void CheckVideoTrack(const std::shared_ptr<PAGExportSession>& session,
                            pag::Composition* composition) {
  constexpr size_t maxVideoTrackNum = 2;

  std::vector<std::pair<pag::TimeRange, size_t>> timeRangeList = {};
  Helper::TraversalLayers(session, composition, pag::LayerType::Image, 0,
  ...
  if (GetMaxCountInSameTimeRange(timeRangeList, &startTime) > maxVideoTrackNum) {
    ...
    Helper::TraversalLayers(session, composition, pag::LayerType::Image, startTime,
                            &GetNameOfLayerWithVideoTrack, &layerNames);
    session->pushWarning(AlertInfoType::VideoTrackPeakNum, std::to_string(startTime));
  }
}

static void CheckContinuousSequenceComposition(const std::shared_ptr<PAGExportSession>& session,
                                               pag::Composition* composition) {
  if (composition->type() != pag::CompositionType::Vector) {
    return;
  }
...
static void CheckBmpSuffix(const std::shared_ptr<PAGExportSession>& session,
                           pag::Composition* composition) {
  if (composition->type() != pag::CompositionType::Vector) {
    return;
  }
  ...
  for (auto layer : static_cast<pag::VectorComposition*>(composition)->layers) {
    ...
    if (layer->type() == pag::LayerType::PreCompose) {
      ...
      if (session->configParam.isTagCodeEnable(pag::TagCode::VideoSequence)) {
        if (StringHelper::IsEndWidthSuffix(layer->name, StringHelper::CompositionBmpSuffix)) {
...
static void CheckLayerNum(const std::shared_ptr<PAGExportSession>& session,
                          std::vector<pag::Composition*>& compositions) {
...
static void CheckTimeRemapPlaySpeed(const std::shared_ptr<PAGExportSession>& session,
...
static void CheckTimeRemapPlayBackwards(const std::shared_ptr<PAGExportSession>& session,
                                        pag::AnimatableProperty<pag::Frame>* timeRemap) {
  for (auto keyFrame : timeRemap->keyframes) {
...
static void CheckLayerImageFillRule(const std::shared_ptr<PAGExportSession>& session,
                                    pag::Layer* layer, void*) {
...
static void CheckImageFillRule(const std::shared_ptr<PAGExportSession>& session,
                               pag::Composition* composition) {
  Helper::TraversalLayers(session, composition, pag::LayerType::Image, &CheckLayerImageFillRule,
                          composition);
}

static void CheckVideoCompositionInUIScenes(const std::shared_ptr<PAGExportSession>& session,
                                            std::vector<pag::Composition*>& compositions) {
...
static bool CompareVideoComposition(const std::shared_ptr<PAGExportSession>& session,
  ...
  if (composition1->frameRate != composition2->frameRate) {
    return false;
  }

  for (pag::Frame frame = 0; frame < composition1->duration && !session->stopExport; frame++) {
    auto image1 = AEHelper::GetCompositionFrameImage(session->itemHMap[composition1->id], frame);
    auto image2 = AEHelper::GetCompositionFrameImage(session->itemHMap[composition2->id], frame);
    if (image1 != image2) {
      return false;
    }
  }
...
static void CheckLayerMarkerCharacter(const std::shared_ptr<PAGExportSession>& session,
                                      pag::Layer* layer) {
...
static void CheckLayerDisplacementMap(const std::shared_ptr<PAGExportSession>& session,
                                      pag::Layer* layer) {
...
static void CheckLayerFlags(const std::shared_ptr<PAGExportSession>& session, pag::Layer* layer) {
  ...
  if (layerFlags & AEGP_LayerFlag_MOTION_BLUR &&
      !session->configParam.isTagCodeEnable(pag::TagCode::LayerAttributesExtra)) {
...
static void CheckTextLayerAnimator(const std::shared_ptr<PAGExportSession>& session,
                                   pag::TextLayer* layer) {
...
static void CheckTextLaterTransform(const std::shared_ptr<PAGExportSession>& session,
                                    pag::TextLayer* layer) {
  ...
  if (layer->transform->scale->animatable()) {
    return;
  }
  ...
  if (layer->sourceText->animatable()) {
    return;
  }
...
static void CheckTextLayerPathOption(const std::shared_ptr<PAGExportSession>& session,
                                     pag::TextLayer* layer) {
...
static void CheckLayerProperty(const std::shared_ptr<PAGExportSession>& session, pag::Layer* layer,
                               void*) {
...
static void CheckLayerProperty(const std::shared_ptr<PAGExportSession>& session,
                               std::vector<pag::Composition*>& compositions) {
  Helper::TraversalLayers(session, compositions, pag::LayerType::Unknown, &CheckLayerProperty,
                          nullptr);
}
...
void CheckBeforeExport(const std::shared_ptr<PAGExportSession>& session,
                       std::vector<pag::Composition*>& compositions,
                       std::vector<pag::ImageBytes*>& images) {
...
void CheckAfterExport(const std::shared_ptr<PAGExportSession>& session,
                      std::vector<pag::Composition*>& compositions) {
  CheckStaticVideoSequence(session, compositions);
}

void CheckGraphicsMemory(const std::shared_ptr<PAGExportSession>& session,
                         const std::shared_ptr<pag::File>& pagFile) {
  auto graphicsMemorySize = CalculateGraphicsMemory(pagFile);
  if (session->configParam.scenes == ExportScenes::UI && graphicsMemorySize > 30 * 1024 * 1024) {
