...
template <typename T>
T GetValue(AEGP_StreamRefH stream, StreamParser<T> parser, const QVariantMap& map = {}) {
  static A_Time time = {0, 100};
  AEGP_StreamType streamType;
  AEHelper::GetSuites()->StreamSuite4()->AEGP_GetStreamType(stream, &streamType);
...
template <typename T>
T GetValue(AEGP_StreamRefH groupStream, int index, StreamParser<T> parser,
           const QVariantMap& map = {}) {
  AEGP_StreamRefH stream = nullptr;
  AEHelper::GetSuites()->DynamicStreamSuite4()->AEGP_GetNewStreamRefByIndex(
      AEHelper::GetPluginID(), groupStream, index, &stream);
  auto result = GetValue(stream, parser, map);
  AEHelper::GetSuites()->StreamSuite4()->AEGP_DisposeStream(stream);
  return result;
}

template <typename T>
T GetValue(AEGP_StreamRefH groupStreamH, const A_char* key, StreamParser<T> parser,
           const QVariantMap& map = {}) {
  AEGP_StreamRefH stream = nullptr;
  AEHelper::GetSuites()->DynamicStreamSuite4()->AEGP_GetNewStreamRefByMatchname(
      AEHelper::GetPluginID(), groupStreamH, key, &stream);
  auto result = GetValue(stream, parser, map);
  AEHelper::GetSuites()->StreamSuite4()->AEGP_DisposeStream(stream);
  return result;
}
