...
static pag::PathHandle ParsePath(const AEGP_StreamVal2& streamValue, const QVariantMap&) {
  ...
  A_long numSegment = 0;
  AEHelper::GetSuites()->MaskOutlineSuite3()->AEGP_GetMaskOutlineNumSegments(stream, &numSegment);
  if (numSegment == 0) {
  ...
  A_Boolean isOpen = 0;
  AEHelper::GetSuites()->MaskOutlineSuite3()->AEGP_IsMaskOutlineOpen(stream, &isOpen);
  if (isOpen == 0) {
...
static pag::TextDocumentHandle ParseTextDocument(const AEGP_StreamVal2&, const QVariantMap& map) {
  ...
  if (!outPath.empty()) {
    std::string fontDstPath = FileHelper::GetDir(outPath);
    fontDstPath = FileHelper::JoinPaths(
        fontDstPath, textDocument->fontFamily + "-" + textDocument->fontStyle + ".ttc");
