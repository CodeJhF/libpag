...
template <typename T>
pag::Keyframe<T>* GetKeyframe(AEGP_StreamRefH stream, StreamParser<T> parser, int index,
  ...
  A_Time time = {};
  Suites->KeyframeSuite4()->AEGP_GetKeyframeTime(stream, index - 1, AEGP_LTimeMode_CompTime, &time);
  keyframe->startTime = AEHelper::AETimeToTime(time, map.value("frameRate", 24.0f).toFloat());
  Suites->KeyframeSuite4()->AEGP_GetKeyframeTime(stream, index, AEGP_LTimeMode_CompTime, &time);
  keyframe->endTime = AEHelper::AETimeToTime(time, map.value("frameRate", 24.0f).toFloat());
  ...
  if (streamType != AEGP_StreamType_NO_DATA) {
    Suites->KeyframeSuite4()->AEGP_GetNewKeyframeValue(PluginID, stream, index - 1, &streamValue);
  }
  QVariantMap mapWithKeyframe = map;
  mapWithKeyframe["keyframeIndex"] = index - 1;
  keyframe->startValue = parser(streamValue.val, mapWithKeyframe);
  if (streamType != AEGP_StreamType_NO_DATA) {
    Suites->StreamSuite4()->AEGP_DisposeStreamValue(&streamValue);
    Suites->KeyframeSuite4()->AEGP_GetNewKeyframeValue(PluginID, stream, index, &streamValue);
  }
  mapWithKeyframe["keyframeIndex"] = index;
  keyframe->endValue = parser(streamValue.val, mapWithKeyframe);
  if (streamType != AEGP_StreamType_NO_DATA) {
    Suites->StreamSuite4()->AEGP_DisposeStreamValue(&streamValue);
  }
  ...
  if (hasSpatial) {
    ...
    Suites->KeyframeSuite4()->AEGP_GetNewKeyframeSpatialTangents(PluginID, stream, index - 1,
    ...
    Suites->KeyframeSuite4()->AEGP_GetNewKeyframeSpatialTangents(PluginID, stream, index,
                                                                 &spatialIn, &spatialOut);
  ...
  Suites->KeyframeSuite4()->AEGP_GetKeyframeInterpolation(stream, index - 1, &tempType, &outType);
  Suites->KeyframeSuite4()->AEGP_GetKeyframeInterpolation(stream, index, &inType, &tempType);

  AEGP_KeyframeFlags flags;
  Suites->KeyframeSuite4()->AEGP_GetKeyframeFlags(stream, index - 1, &flags);
  ...
  if (outType == AEGP_KeyInterp_HOLD) {
    ...
    auto averageSpeedList = GetAverageSpeed(stream, index, dimensionality);
    for (int i = 0; i < dimensionality; i++) {
      AEGP_KeyframeEase tempEase = {};
      AEGP_KeyframeEase easeOut = {};
      AEGP_KeyframeEase easeIn = {};
      Suites->KeyframeSuite4()->AEGP_GetKeyframeTemporalEase(stream, index - 1, i, &tempEase,
                                                             &easeOut);
      Suites->KeyframeSuite4()->AEGP_GetKeyframeTemporalEase(stream, index, i, &easeIn, &tempEase);
      float averageSpeed =
...
template <typename T>
pag::Property<T>* GetAnimatableProperty(AEGP_StreamRefH stream, StreamParser<T> parser,
                                        const QVariantMap& map, uint8_t dimensionality) {
  A_long keyframeNum = 0;
  AEHelper::GetSuites()->KeyframeSuite4()->AEGP_GetStreamNumKFs(stream, &keyframeNum);
  std::vector<pag::Keyframe<T>*> keyframes;
  for (int i = 1; i < keyframeNum; i++) {
    auto keyframe = GetKeyframe(stream, parser, i, map, dimensionality);
...
template <typename T>
pag::Property<T>* GetProperty(AEGP_StreamRefH stream, StreamParser<T> parser,
                              const QVariantMap& map = {}, uint8_t dimensionality = 1) {
  if (stream == nullptr) {
    return nullptr;
  }
  CheckStreamExpression(stream);
  pag::Property<T>* property;
  A_long numKeys = 0;
  AEHelper::GetSuites()->KeyframeSuite4()->AEGP_GetStreamNumKFs(stream, &numKeys);
  if (numKeys <= 1) {
    property = new pag::Property<T>();
    property->value = GetValue(stream, parser, map);
  } else {
    return GetAnimatableProperty(stream, parser, map, dimensionality);
  }
  return property;
}

template <typename T>
pag::Property<T>* GetProperty(AEGP_StreamRefH groupStreamH, int index, StreamParser<T> parser,
                              const QVariantMap& map = {}, uint8_t dimensionality = 1) {
  const auto& Suites = AEHelper::GetSuites();
  const auto& PluginID = AEHelper::GetPluginID();

  AEGP_StreamRefH streamH = nullptr;
  Suites->DynamicStreamSuite4()->AEGP_GetNewStreamRefByIndex(PluginID, groupStreamH, index,
                                                             &streamH);
  auto result = GetProperty(streamH, parser, map, dimensionality);
  Suites->StreamSuite4()->AEGP_DisposeStream(streamH);
  return result;
}

template <typename T>
pag::Property<T>* GetProperty(AEGP_StreamRefH groupStreamH, const A_char* key,
  ...
  AEGP_StreamRefH streamH = nullptr;
  Suites->DynamicStreamSuite4()->AEGP_GetNewStreamRefByMatchname(PluginID, groupStreamH, key,
                                                                 &streamH);
  auto result = GetProperty(streamH, parser, map, dimensionality);
  Suites->StreamSuite4()->AEGP_DisposeStream(streamH);
  return result;
...
template <typename T>
pag::Property<T>* GetProperty(const AEGP_LayerH& layerH, AEGP_LayerStream layerStream,
                              StreamParser<T> parser, const QVariantMap& map = {},
                              uint8_t dimensionality = 1) {
  const auto& Suites = AEHelper::GetSuites();
  const auto& PluginID = AEHelper::GetPluginID();
  ...
  AEGP_StreamRefH streamH = nullptr;
  Suites->StreamSuite4()->AEGP_GetNewLayerStream(PluginID, layerH, layerStream, &streamH);
  auto result = GetProperty(streamH, parser, map, dimensionality);
  Suites->StreamSuite4()->AEGP_DisposeStream(streamH);
  return result;
}
...
template <typename T>
pag::Property<T>* GetProperty(const AEGP_MaskRefH& maskH, AEGP_MaskStream maskStream,
                              StreamParser<T> parser, const QVariantMap& map = {},
                              uint8_t dimensionality = 1) {
  const auto& Suites = AEHelper::GetSuites();
  const auto& PluginID = AEHelper::GetPluginID();
  ...
  AEGP_StreamRefH streamH = nullptr;
  Suites->StreamSuite4()->AEGP_GetNewMaskStream(PluginID, maskH, maskStream, &streamH);
  auto result = GetProperty(streamH, parser, map, dimensionality);
  Suites->StreamSuite4()->AEGP_DisposeStream(streamH);
  return result;
}
