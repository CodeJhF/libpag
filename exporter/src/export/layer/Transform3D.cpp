...
static pag::Property<pag::Point3D>* GetOrientationKeyframe(const AEGP_LayerH& layerH,
                                                           float frameRate) {
  auto orientation =
      GetProperty(layerH, AEGP_LayerStream_ORIENTATION, AEStreamParser::Point3DParser);
  if (!orientation->animatable()) {
    return orientation;
  }
  ...
  Suites->StreamSuite4()->AEGP_GetNewLayerStream(PluginID, layerH, AEGP_LayerStream_ORIENTATION,
                                                 &streamH);
  ...
  for (auto keyframe :
    ...
    for (auto frame = keyframe->startTime + step; frame < keyframe->endTime; frame += step) {
      ...
      Suites->KeyframeSuite3()->AEGP_InsertKeyframe(streamH, AEGP_LTimeMode_CompTime, &time,
                                                    &index);
      if (index > 0) {
  ...
  delete orientation;
  auto newOrientation = GetProperty(streamH, AEStreamParser::Point3DParser);

  for (int i = static_cast<int>(list.size()) - 1; i >= 0; i--) {
    Suites->KeyframeSuite3()->AEGP_DeleteKeyframe(streamH, list.at(i));
  }
...
pag::Transform3D* GetTransform3D(const AEGP_LayerH& layerH, float frameRate) {
  const A_long PluginID = AEHelper::GetPluginID();
  const auto Suites = AEHelper::GetSuites();
  ...
  if (dimensionSeparated != 0) {
    ...
    Suites->DynamicStreamSuite4()->AEGP_GetSeparationFollower(position, 0, &xPosition);
    ...
    Suites->DynamicStreamSuite4()->AEGP_GetSeparationFollower(position, 1, &yPosition);
    ...
    Suites->DynamicStreamSuite4()->AEGP_GetSeparationFollower(position, 1, &zPosition);
    transform->zPosition = GetProperty(zPosition, AEStreamParser::FloatParser);
    Suites->StreamSuite4()->AEGP_DisposeStream(zPosition);
  } else {
    transform->position = GetProperty(position, AEStreamParser::Point3DParser);
  }
  Suites->StreamSuite4()->AEGP_DisposeStream(position);
  transform->anchorPoint =
      GetProperty(layerH, AEGP_LayerStream_ANCHORPOINT, AEStreamParser::Point3DParser);
  transform->scale =
      GetProperty(layerH, AEGP_LayerStream_SCALE, AEStreamParser::Scale3DParser, {}, 3);
  transform->orientation = GetOrientationKeyframe(layerH, frameRate);
  transform->xRotation =
      GetProperty(layerH, AEGP_LayerStream_ROTATE_X, AEStreamParser::FloatParser);
  transform->yRotation =
      GetProperty(layerH, AEGP_LayerStream_ROTATE_Y, AEStreamParser::FloatParser);
  transform->zRotation =
      GetProperty(layerH, AEGP_LayerStream_ROTATE_Z, AEStreamParser::FloatParser);
  transform->opacity =
