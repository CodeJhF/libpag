...
void ExportCompositionInfoModel::setAEResource(const std::shared_ptr<AEResource>& resource) {
  ...
  size = AEHelper::GetItemDimensions(resource->itemH);
...
void ExportCompositionInfoModel::setExportAsBmp(int row, bool exportAsBmp) {
  ...
  if (exportAsBmp) {
    ...
    AEHelper::SetItemName(item.resource->itemH, newName);
    item.resource->name = newName;
    item.resource->isExportAsBmp = true;
  } else {
    std::string newName = item.resource->name;
    auto pos = newName.find(StringHelper::CompositionBmpSuffix,
                            newName.size() - StringHelper::CompositionBmpSuffix.size());
  ...
  for (; index < items.size(); index++) {
    ...
    if (currentItem.level > item.level) {
      if (exportAsBmp) {
        ...
        this->resource->composition.exportAsBmpMap[currentItem.resource->ID] = true;
      } else {
        ...
        if (currentItem.resource->isExportAsBmp) {
          ...
          this->resource->composition.exportAsBmpMap[currentItem.resource->ID] =
...
QVariant ExportCompositionInfoModel::data(const QModelIndex& index, int role) const {
  ...
  switch (role) {
    ...
    case static_cast<int>(ExportCompositionInfoModelRoles::ExportAsBmpRole): {
      return this->resource->composition.exportAsBmpMap[item.resource->ID];
    }
